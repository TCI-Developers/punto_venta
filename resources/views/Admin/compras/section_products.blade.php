<div class="card-body">
    @if(!isset($compra) || isset($compra) && $compra->status == 1)
        <label for="product_id" class="col-12" wire:ignore>Productos
            <select name="product_id[]" id="product_id" class="form-control selectpicker" data-live-search="true" show-tick data-style="btn-secondary"
            data-size="10" data-selected-text-format="count" wire:change="selectProducts" wire:model="select_product" multiple required>
                                    ...
                                    <tr class="item_product">
                                        ...
                                        @if(Auth::User()->hasAnyRole(['root', 'admin']) && $compra->status == 1)
                                            <td class="text-center">
                                                <button type="button" class="btn btn-warning btn-sm" onclick="btnEditEntrada(this)"><i class="fa fa-edit"></i></button>
                                                <button type="button" class="btn btn-success btn-sm btnOkEntrada displayNone"><i class="fa fa-check"></i></button>

        ...
        @endif
            ...
            <div class="card-body">
                ...
                <div class="row form-group">
                    <label for="proveedor_id" class="col-lg-4 col-sm-12" wire:ignore>Proveedor*
                        <select class="form-control selectpicker" name="proveedor_id" id="proveedor_id" data-live-search="true"
                            onchange="diasCredito(this)" {{$status ?? ''}} required>
                            <option value=""></option>
                            @forelse($proveedores as $item)
                                <option value="{{$item->id}}" days_credit="{{$item->credit_days}}" {{isset($compra) && $compra->proveedor_id == $item->id ? 'selected':'' }}>{{$item->name}}</option>
                            @empty
                            @endif
                        </select>
                    </label>
            ...
            <div class="card-footer text-right">
                @if(isset($compra))
                    ...
                    @elseif($compra->status == 2)
                    <button type="button" class="btn btn-info" onclick="btnSolicitar()"><i class="fa fa-circle"></i> Solicitar</button>
